#!/usr/bin/env python3
"""
Generate an RP2350 ARM benchmark firmware for the mdrp2354 showcase app.
...
References:
  - roms/gen_blinky.py / roms/gen_lcd_demo.py (builder template)
  - wrk_docs/2026.04.13 - LLD - Emulator Showcase App.md §8, §10.2
  - crates/mdrp2354/src/tests.rs (encoder reference for Thumb-32
...
# LDMIA.W, LDR.W imm12) match crates/mdrp2354/src/tests.rs so their
...
def thumb_b_w(offset):
    ...
    and roms/gen_lcd_demo.py::thumb_b_w.
    """
...
# crates/mdrp2354/src/tests.rs, which has unit tests for each of these.
# -----------------------------------------------------------------------------
...
def thumb_mul_w(rd, rn, rm):
    ...
    Matches crates/mdrp2354/src/tests.rs::encode_mul_w.
    """
...
def thumb_sdiv(rd, rn, rm):
    ...
    Matches crates/mdrp2354/src/tests.rs::encode_sdiv.
    """
    assert 0 <= rd <= 15 and 0 <= rn <= 15 and 0 <= rm <= 15
    hw0 = 0xFB90 | rn
...
def thumb_clz(rd, rm):
    ...
    Matches crates/mdrp2354/src/tests.rs::clz_w (which shows the raw
...
def thumb_ldmia_w(rn, writeback, reglist):
    ...
    the register list, M is LR. Matches crates/mdrp2354/src/tests.rs::
