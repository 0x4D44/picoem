...
- `mdpicoem-common` — shared primitives crate: `Memory`, `ClockTree` math, `Pacer`, PIO primitive types (`PioBlock`/`StateMachine`), divider/FIFO, and `threaded::{SpinBarrier, SpscQueue}` (x86_64 Windows only).
- `mdpicoem-devices` — off-chip device models used by harnesses and apps: `psram.rs`, `lcd.rs`, `i2s_capture.rs`. Depended on by `mdrp2040` (PSRAM compat).
- `mdrp2350` — the RP2350/RP2354 emulator library (dual Cortex-M33 + RISC-V Hazard3 cores, bus, memory, SIO, PIO, DMA, full peripheral suite, clocks, pacer, threaded runtime).
- `mdrp2350app` — the TUI demo app driving `mdrp2350`.
- `mdrp2040` — the RP2040 emulator library (dual Cortex-M0+ cores, bus, memory, SIO, PIO, clocks, threaded runtime).
- `mdrp2040app` — the TUI demo app driving `mdrp2040`.
- `mdpicoem-harness` — differential test binaries (QEMU diff + probe-rs diff variants per chip, softfloat diff, paced benchmark, silicon oracles, PicoGUS replay, OneROM oracles, test_silicon orchestrator).
...
- **`crates/mdpicoem-common`** — shared primitives pulled out in Phase 2: `Memory` (with `new()` for RP2350 sizes and `with_sizes(rom, sram)` for RP2040 / future chips), `ClockTree` math, `Pacer` (`x86_64`-only), `PioBlock`/`StateMachine`, `Divider`, `Fifo`. `threaded::{SpinBarrier, SpscQueue}` (x86_64 Windows only) are the cross-chip thread-coordination primitives used by each chip's `threaded/` runtime. Both chip crates depend on this. Note: a former `Peripheral` trait was removed (zero impls workspace-wide); peripherals now use inherent methods invoked from `Bus` per `wrk_docs/2026.04.15 - HLD - RP2040 Peripheral Coverage V7.md` §5.1.
- **`crates/mdpicoem-devices`** — off-chip device emulation shared by harnesses and apps: `psram.rs` (HyperRAM-style external PSRAM, consumed by `mdrp2040` for `test_psram` compat), `lcd.rs`, `i2s_capture.rs`. Version-pinned 0.7.x at time of writing.
- **`crates/mdrp2350`** — the RP2350/RP2354 emulator core (dual Cortex-M33 / ARMv8-M Mainline, bus, 520 KB SRAM, 32 KB bootrom, XIP flash, clocks, SIO, PIO, FPU, coprocessors, 16-channel DMA, and the peripheral suite: TIMER0/1, TICKS, POWMAN, UART0/1, SPI0/1, I2C0/1, PWM, ADC, WATCHDOG, OTP, TRNG, SHA256, PSM, IO_BANK0, PADS_BANK0, CoreSight). A parallel RISC-V Hazard3 core (`core_riscv/`) shares the same bus. The `threaded/` subtree (x86_64 Windows only, feature `threading`, on by default) provides a 6-thread worker runtime selected by `ExecutionModel::Threaded`. All the RP2350 hot path lives here.
...
- **`crates/mdrp2040`** — the RP2040 emulator core (dual Cortex-M0+ / ARMv6-M, bus, 264 KB SRAM across 4 striped + 2 scratch banks, 16 KB bootrom, **no onboard flash**, clocks, SIO, PIO). No FPU/coprocessors/secure world. The `threaded/` subtree (same platform/feature gating as mdrp2350) provides a 3-thread runtime selected by `ExecutionModel::Threaded`. Depends on `mdpicoem-devices` for PSRAM.
...
- **`crates/mdpicoem-harness`** — all test binaries (see "Testing Topology" below). Binaries are chip-suffixed: `qemu_diff_m33` vs `qemu_diff_m0plus`, `probe_diff_rp2350` vs `probe_diff_rp2040`, `silicon_periph_diff_rp2350` vs `silicon_periph_diff_rp2040`, etc.
...
**The real entry points are `mdrp2350app` and `mdrp2040app`.** The workspace has no top-level binary; build/run via `cargo run -p mdrp2350app` or `cargo run -p mdrp2040app`.
...
- **`lib.rs`** — `Emulator` aggregates two `CortexM0Plus` cores and a `Bus`. Public API mirrors mdrp2350 (`step`/`run`/`load_bootrom`/`load_image`/`gpio_*`). `EmulatorBuilder::build() -> Result<Emulator, ConfigError>` with `step_quantum` (now actively honoured — drains up to N master-clock cycles per `step()` call, default 64) and an `execution(ExecutionModel)` selector (`Serial` / `Threaded`, where `Threaded` requires the `threading` feature and x86_64 Windows).
...
- **`threading` feature** — on by default, but the `Threaded` variant is further `cfg`-gated to `x86_64 + windows`. On any other target `Builder::build()` with `Threaded` returns `ConfigError::ThreadingUnavailable`. Keep Serial as the default for new harnesses and oracles; opt into Threaded only where you specifically want to exercise the runtime.
- **Oracles validate Serial.** QEMU diff, probe diff, silicon cycle/periph/isr/dualcore — all run Serial. Use the `dual_model` integration test (`cargo test -p mdrp2350 --test dual_model --features threading`, same shape on mdrp2040) to assert Serial/Threaded end-state parity.
...
1. **Unit tests**
   ...
   - **Threaded-runtime integration tests** — `crates/mdrp2350/tests/dual_model.rs` + `crates/mdrp2040/tests/dual_model.rs` assert Serial/Threaded end-state parity (require `--features threading`); `crates/mdrp2350/tests/execution_model.rs` + `crates/mdrp2040/tests/execution_model.rs` cover panic-containment via `ExecutionModel::Threaded` + `inject_panic_for_testing` (require `--features threading,testing`). `crates/mdrp2350/tests/hello_riscv_blinky.rs` is the Hazard3 firmware smoke.
...
5. **`probe_diff_rp2040`** — M0+ ISA differential oracle against live RP2040 silicon via SWD (probe-rs 0.31). Unlike `qemu_diff_m0plus`, its `is_m0plus_silicon_safe` filter admits the M0+ Thumb-32 subset (`BL`, `MRS`/`MSR` for `PRIMASK`/`CONTROL`, `DSB`/`DMB`/`ISB`), so silicon is the only oracle for those encodings. No cycle comparison — M0+ has no DWT CYCCNT, so `--cycles` is intentionally absent. A post-step `UndefOnSilicon` diagnostic flags filter-gap escapes (PC landed in the RP2040 bootrom range `< 0x0000_4000` after the step) distinctly from `[FAIL]`/`[SKIP]`. `--probe <VID:PID:SERIAL>` disambiguates on hosts with multiple probes attached. Requires a Pico debug probe on a Pico V1 / RP2040 board — not for CI. `cargo run -p mdpicoem-harness --release --bin probe_diff_rp2040`.
...
15. **`picogus_diff_rp2040`** (+ **`picogus_probe_pc`**) — PicoGUS ISA-bus replayer. Reads a CSV trace captured from a patched DOSBox-X and drives synthetic ISA cycles into an `mdrp2040::Emulator` running the real PicoGUS firmware, sampling the I2S pins per cycle and writing decoded stereo PCM to a WAV at the end. Integration goal is "does Monkey Island sound right". See `wrk_docs/2026.04.14 - HLD - PicoGUS Integration.md` and the 2026-04-2x journals for audio-quality progression. `picogus_probe_pc` is the live-silicon variant.
...
17. **RISC-V Hazard3 oracles** — `qemu_diff_riscv32` (QEMU RV32IMC-Zba-Zbb-Zbs diff for mdrp2350's Hazard3 core), `riscv_probe_spike` (spike reference diff). Same role as the M33 oracles but for the parallel RISC-V path. `probe_csrrw_riscv32` is a one-shot Stage-6 probe (now resolved) — see its file header; not an oracle.
...
RUST_LOG=debug cargo run -p mdpicoem-harness --bin qemu_diff_m33
```

## Non-Obvious Conventions

- **Bank contention model (RP2040 Serial only, deprecated-in-place)**: in the Serial `step` path, core 0 runs first (recording which downstream port it touched in `core0_port`), then core 1 runs with `contention_check_active` — any same-port access adds +1 cycle. If core 1 is halted, contention checking is skipped entirely. This is why the `Bus` struct carries both flags. **Do not extend this model to mdrp2350, and do not invest further on mdrp2040**: the perf gap against real silicon (see `paced_bench_rp2*`) swamps the contention-accuracy gain by an order of magnitude, contention cycles are virtual (they don't consume host compute), and the Threaded `ExecutionModel` bypasses the serial-interleave path entirely (each core runs on its own worker thread, barrier-synchronised at the quantum boundary — no `contention_check_active` in that path). mdrp2350 `contention`/`stress` bench workloads are dual-core-compute-only by design. Rationale: `wrk_journals/2026.04.15 - JRN - Contention Modelling Declined.md`.
...
- **Probe serial → DUT mapping (hard-wired):**
