...
8. **`silicon_periph_diff_rp2350`** — end-state differential oracle for peripheral state against live RP2354 silicon. Each scenario applies an identical MMIO setup sequence and a CYCCNT-measured sysclk window to both silicon (via probe-rs) and a `step_quantum=1` emulator (via `Emulator::run` with halted cores), then diffs a scenario-declared set of observable registers and pins. Covers PIO (register + FIFO + pad state), PLL LOCK timing, and is shaped to absorb future SIO-DIV scenarios without new infrastructure. Same HW prerequisite as the cycle oracle — Pico probe + RP2354, not for CI. `cargo run -p mdpicoem-harness --release --bin silicon_periph_diff_rp2350`; see `wrk_docs/2026.04.15 - HLD - Silicon Peripheral and Cycle Oracles.md` §Oracle 1 for the catalog.
