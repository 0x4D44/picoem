...
use rp2350_emu_tui::sim::{self, FirmwareBytes};
use rp2350_emu_tui::snapshot::Snapshot;
use rp2350_emu_tui::ui;
...
fn resolve_firmware_path(arg: &str) -> PathBuf {
    ...
    match arg {
        "blinky" => manifest_dir.join("../../roms/rp2350/blinky.bin"),
        "benchmark" => manifest_dir.join("../../roms/rp2350/benchmark.bin"),
        "lcd" => manifest_dir.join("../../roms/rp2350/lcd_demo.bin"),
        path => PathBuf::from(path),
    }
}

fn bootrom_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../roms/rp2350/bootrom-combined.bin")
}
...
mod tests {
    ...
    fn preset_aliases_resolve_under_manifest_dir() {
        assert_eq!(
            ...
            manifest_dir().join("../../roms/rp2350/blinky.bin")
        );
        assert_eq!(
            resolve_firmware_path("benchmark"),
            manifest_dir().join("../../roms/rp2350/benchmark.bin")
        );
        assert_eq!(
            resolve_firmware_path("lcd"),
            manifest_dir().join("../../roms/rp2350/lcd_demo.bin")
        );
    }
