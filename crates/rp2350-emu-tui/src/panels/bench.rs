...
//!
//! Draws the six-row section table from [`Snapshot::benchmark`] with a
//! color per row: green for Δ == 0, red for Δ != 0, yellow for the
//! stall indicator (LLD §14 "Closed").
//!
...
fn build_lines(report: &BenchmarkReport) -> Vec<Line<'static>> {
    ...
    if let Some(phase) = report.stall {
        // LLD §8.2: stall surfaces in the panel as "stalled at phase 0xNN
        // after Ns". Yellow per §14.
        lines.push(Line::from(Span::styled(
