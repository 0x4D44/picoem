...
use rp2350_emu_tui::firmware::REFERENCE_TABLE;
use rp2350_emu_tui::sim::{self, FirmwareBytes};
use rp2350_emu_tui::snapshot::Snapshot;

fn roms_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../roms/rp2350")
}
...
fn load_firmware(name: &str) -> FirmwareBytes {
    ...
    FirmwareBytes {
        bootrom: std::fs::read(roms.join("bootrom-combined.bin"))
            .expect("roms/rp2350/bootrom-combined.bin missing"),
        flash: std::fs::read(roms.join(name))
            .unwrap_or_else(|_| panic!("roms/rp2350/{name} missing")),
    }
}
