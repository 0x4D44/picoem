...
fn apply_init_regs_emu(emu: &mut rp2350_emu::Emulator, init_regs: &[(IsrReg, u32)]) {
    for &(reg, val) in init_regs {
        match reg {
            IsrReg::Cpacr => {
...
fn scenario_preamble_emu(emu: &mut rp2350_emu::Emulator, name: &str) {
    match name {
        "isr_eager_fp_save" => {
...
fn read_observable_emu(emu: &mut rp2350_emu::Emulator, obs: IsrObservable, msp: u32) -> u32 {
    match obs {
