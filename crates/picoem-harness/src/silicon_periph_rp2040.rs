// silicon_periph_rp2040 — peripheral-state oracle library for rp2040_emu
...
//   * Emulator is `rp2040_emu::Emulator` (Cortex-M0+).
...
/// `setup` in order on both the HW `Core` and a fresh `rp2040_emu::Emulator`,
...
// `rp2040_emu/src/bus/mod.rs:569`), so this is a *green round-trip*, not
...
fn sample_pins_emu(emu: &mut rp2040_emu::Emulator, mask: u32) -> (u32, u32) {
...
mod tests {
    ...
    /// Dry-run the assembled sled through `rp2040_emu::Emulator` and assert
