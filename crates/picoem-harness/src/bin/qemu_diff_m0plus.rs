// QEMU differential test runner — rp2040_emu (Cortex-M0+) oracle.
//
...
//   * Emulator: `rp2040_emu::CortexM0Plus` + `rp2040_emu::Bus`.
//
...
// The fuzz generator lives in `picoem_harness::lib`; we just select
...
use picoem_harness::gdb_client::{GdbClient, QemuProcess, QemuProfile};
use picoem_harness::m0plus::{Bus as M0Bus, CortexM0Plus};
use picoem_harness::{
...
///     [`picoem_harness::m0plus_admits_wide`].
fn is_m0plus_safe(tc: &TestCase) -> bool {
    ...
    if let Some(hw1) = tc.hw1
        && !picoem_harness::m0plus_admits_wide(tc.opcode, hw1)
    {
        return false;
    }
    true
...
/// Execute the test on our rp2040_emu emulator and read back post-state.
///
/// Parallels `picoem_harness::run_one_emu` but uses the M0+ core and
...
mod tests {
    use super::*;
    use picoem_harness::thumb32_gen::{enc_t32_bl, enc_t32_mrs, enc_t32_msr};
