// silicon_isr_diff_rp2040 — ISR differential oracle for rp2040_emu vs.
// real RP2040 silicon.
//
...
use picoem_harness::{ISR_IMAGE_BASE, ISR_STACK_TOP};
...
fn run() -> Result<i32, Box<dyn std::error::Error>> {
    ...
    let selected: Vec<&isr_scenarios_rp2040::IsrScenario> = isr_scenarios_rp2040::SCENARIOS
        .iter()
        .filter(|s| {
            picoem_harness::silicon_oracle::name_matches_filter(s.name, args.filter.as_deref())
        })
        .collect();
