...
use picoem_harness::gdb_client::{GdbClient, QemuProcess, QemuProfile, REG_RV_PC, REG_RV_X0};
use picoem_harness::riscv_gen::{
    self, OPC_OP_IMM, OPC_SYSTEM, RiscvClass, RiscvTestCase, SCRATCH_BASE, TRAP_STUB, encode_csr,
    encode_i_type, encode_s_type,
};

use rp2350_emu::{Arch, Config, EmulatorBuilder};
...
// DRAM; the rp2350_emu bus aliases `0x8xxx_xxxx` → SRAM via
...
fn warl_mask(csr: u16, v: u32) -> u32 {
    match csr {
        ...
        // mtimecmp=0 and mtime is running), while rp2350_emu has no CLINT
...
fn read_gprs_emu(emu: &rp2350_emu::Emulator) -> [u32; 32] {
    let mut out = [0u32; 32];
...
fn apply_reg_pre_emu(emu: &mut rp2350_emu::Emulator, tc: &RiscvTestCase) {
    let h = emu.core_riscv_mut(0);
    for r in 1u8..32 {
        h.set_gpr(r, 0);
    }
...
fn read_emu_scratch(emu: &rp2350_emu::Emulator, addr: u32, len: usize) -> Vec<u8> {
    // Canonical SRAM alias mask from `rp2350_emu::bus` decode — strips the
