...
use picoem_harness::{onerom_serving_oracle, onerom_sync};
...
fn main() -> ExitCode {
    ...
    // Capture the SRAM shadow. The real DMA peripheral now drives the
    // serving chain on its own — no harness-side priming needed.
    let mut oracle =
