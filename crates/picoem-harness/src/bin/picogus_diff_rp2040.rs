// PicoGUS trace replayer — rp2040_emu (Cortex-M0+) harness.
//
...
// cycles into our `rp2040_emu::Emulator`, stepping virtual time forward
...
// [`picoem_harness::picogus_pins`]. It is cross-checked against the
...
use picoem_devices::{I2sCapture, Psram};
use picoem_harness::picogus_pins::{
...
fn replay_with_coverage(
    ...
    let apply_replay_pokes = |emu: &mut rp2040_emu::Emulator, pokes: &[(u32, u8)]| {
        for &(a, v) in pokes {
            let word_addr = a & !3;
            let shift = (a & 3) * 8;
    ...
    eprintln!(
        "Usage:\n  \
         ...
         --out        Optional. Path for the captured I2S WAV. Default:\n              \
                      crates/picoem-harness/oracles/picogus_<trace_stem>.wav.\n\
         --firmware-mode N\n              \
...
impl UartDrain {
    ...
    pub fn drain_emu(&mut self, emu: &mut rp2040_emu::Emulator) {
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(slot) = args.firmware_mode {
        let addr = rp2040_emu::bus::WATCHDOG_BASE + 0x18;
        eprintln!(
    ...
    let out_path = args
        .out
        .clone()
        .unwrap_or_else(|| picoem_harness::default_out_path(&args.trace));
    ...
    if !poke_spec.is_empty() || extra_postroll_secs > 0.0 {
        ...
        let apply_pokes = |emu: &mut rp2040_emu::Emulator, pokes: &[(u32, u8)]| {
            for &(a, v) in pokes {
                let word_addr = a & !3;
                let shift = (a & 3) * 8;
                let w = emu.peek(word_addr);
    ...
    //  picoem-common/src/pio/mod.rs::fstat().)
...
";
    ...
    fn capturing_sink_forwards_ticks_to_i2s_capture() {
        use picoem_harness::picogus_pins::{I2S_BCLK, I2S_LRCLK};
    ...
    fn capturing_sink_stamps_edges_in_sysclks_not_ticks() {
        use picoem_harness::picogus_pins::I2S_LRCLK;
    ...
    /// End-to-end with a real `rp2040_emu::Emulator`: a branch-to-self
    ...
    fn i2s_capture_set_sys_clk_hz_rescales_rate() {
        use picoem_devices::i2s_capture::I2sCapture;
