...
impl GlueDma {
    ...
    pub fn tick(&mut self, bus: &mut Bus) {
        ...
        // (`rp2350_emu::dma` CTRL bit-field map) the real DMA now decodes
...
mod tests {
    ...
    // ----- RP2350 V6 CTRL field positions (mirrors rp2350_emu::dma). -----
    // Pinned here so a future rp2350_emu DMA refactor that shifts fields
    ...
    // `crates/rp2350_emu/src/dma.rs` CTRL bit-field map.
    const V6_EN: u32 = 1 << 0;
    ...
    /// RP2350 V6 CTRL builder.  Keeps the test independent of rp2350_emu's
    ...
    /// actually advances it.  Mirrors the sequence `rp2350_emu::dma`'s own
    ...
    /// V6 bit positions regression: once the rp2350_emu DMA CTRL layout
