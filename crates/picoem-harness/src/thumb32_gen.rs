...
/// range (±2^24), but going wider risks PC wraps that QEMU and rp2040_emu
...
/// MSR writes for sysm 8 (MSP), 9 (PSP), and 20 (CONTROL) — rp2040_emu is
/// spec-correct on those paths (Stage E.1 verdict in
/// `wrk_journals/2026.04.29 - JRN - M0+ T32 Randomised Fuzz Generator.md`),
/// and curated unit tests in `crates/rp2040_emu/src/tests.rs` (mod
...
fn fuzz_m0plus_barrier(count: usize, rng: &mut StdRng) -> Vec<TestCase> {
    let mut t = Vec::with_capacity(count);
    // Filter admits any op nibble; rp2040_emu UNDEFs op∉{4,5,6} (DSB/DMB/ISB),
