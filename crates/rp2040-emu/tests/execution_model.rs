...
//!
//! Mirrors `rp2350_emu/tests/execution_model.rs` but simpler: RP2040 has
//! only three worker variants (`Core0`, `Core1`, `Coord`), no PIO
//! worker (PIO runs on the coordinator in Stage 3b.4).
//!
//! Run with `cargo test -p rp2040_emu --features testing` — the
...
use rp2040_emu::{Config, EmulatorBuilder, ExecutionModel};
...
fn worker_panic_surfaces_as_error() {
    use rp2040_emu::{EmulatorError, WorkerName};
