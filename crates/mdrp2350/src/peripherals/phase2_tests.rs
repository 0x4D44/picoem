...
fn uart0_narrow_byte_write_to_dr_pushes_one_byte() {
    ...
    bus.write32(UART0_BASE + UARTCR, UARTCR_UARTEN | UARTCR_TXE);
    // Byte write via narrow dispatch.
    bus.write8(UART0_BASE + UARTDR, 0x5A);
...
fn uart0_irq_routed_to_bus_irq_pending() {
    ...
    bus.write32(UART0_BASE + UARTLCR_H, UARTLCR_H_FEN);
    bus.write32(UART0_BASE + UARTCR, UARTCR_UARTEN | UARTCR_TXE);
    bus.write32(UART0_BASE + UARTIMSC, UART_INT_TX);
    bus.write32(UART0_BASE + UARTDR, 0x55);
...
fn spi0_sspsr_at_reset_reports_tfe() {
    let mut bus = Bus::new();
    let sr = bus.read32(SPI0_BASE + SSPSR);
...
fn spi0_loopback_rx_matches_tx() {
    ...
    bus.write32(SPI0_BASE + SSPCR0, 7);
    bus.write32(SPI0_BASE + SSPCR1, SSPCR1_SSE | SSPCR1_LBM);
    bus.write32(SPI0_BASE + SSPDR, 0xA5);
    // Narrow read via read8 must pop one byte from RX FIFO.
    let got = bus.read32(SPI0_BASE + SSPDR);
...
fn spi0_byte_narrow_read_from_dr() {
    ...
    bus.write32(SPI0_BASE + SSPCR0, 7);
    bus.write32(SPI0_BASE + SSPCR1, SSPCR1_SSE | SSPCR1_LBM);
    bus.write32(SPI0_BASE + SSPDR, 0x33);
    let b = bus.read8(SPI0_BASE + SSPDR);
    assert_eq!(b, 0x33);
}
...
fn i2c0_bus_scan_nacks_with_abrt_7b() {
    ...
    bus.write32(I2C0_BASE + IC_TAR, 0x7F);
    bus.write32(I2C0_BASE + IC_ENABLE, 1);
    bus.write32(I2C0_BASE + IC_DATA_CMD, (1 << 8) | (1 << 9)); // READ + STOP
    // Expect TX_ABRT in raw interrupt status.
    let raw = bus.read32(I2C0_BASE + IC_RAW_INTR_STAT);
    assert_ne!(raw & INT_TX_ABRT, 0, "TX_ABRT must latch on NACK");
    // Abort source: 7-bit no ack.
    let src = bus.read32(I2C0_BASE + IC_TX_ABRT_SOURCE);
...
fn i2c0_narrow_write_to_data_cmd() {
    ...
    bus.write8(I2C0_BASE + IC_DATA_CMD, 0x55);
    let raw = bus.read32(I2C0_BASE + IC_RAW_INTR_STAT);
...
fn adc_cs_en_latches_ready() {
    let mut bus = Bus::new();
    bus.write32(ADC_BASE + ADC_CS, CS_EN);
    let cs = bus.read32(ADC_BASE + ADC_CS);
...
fn adc_one_shot_completes_via_tick() {
    ...
    bus.write32(ADC_BASE + ADC_CS, CS_EN | CS_START_ONCE | (3 << 12));
...
fn adc_fifo_irq_routed_when_unmasked() {
    ...
    bus.write32(ADC_BASE + FCS, FCS_EN | (1 << FCS_THRESH_SHIFT));
    bus.write32(ADC_BASE + ADC_INTE, 1);
    bus.write32(ADC_BASE + ADC_CS, CS_EN | CS_START_ONCE);
    bus.tick_peripherals(500);
    for core in 0..2 {
        assert_ne!(
...
fn pwm_slice0_wrap_latches_intr_bit() {
    ...
    bus.write32(PWM_BASE + 0x00, CSR_EN); // SLICE_CSR
    bus.write32(PWM_BASE + 0x10, 100); // SLICE_TOP
    bus.write32(PWM_BASE + PWM_EN, 1);
    // 101 sys_clks → one wrap.
    bus.tick_peripherals(101);
    let intr = bus.read32(PWM_BASE + PWM_INTR);
...
fn pwm_slice0_inte0_routes_to_wrap0_irq() {
    let mut bus = Bus::new();
    bus.write32(PWM_BASE + 0x00, CSR_EN);
    bus.write32(PWM_BASE + 0x10, 50);
    bus.write32(PWM_BASE + PWM_EN, 1);
    bus.write32(PWM_BASE + INTE0, 1);
...
fn io_bank0_gpio0_ctrl_roundtrip() {
    ...
    bus.write32(IO_BANK0_BASE + 0x04, 0x0000_0002);
    assert_eq!(bus.read32(IO_BANK0_BASE + 0x04), 0x0000_0002);
}
...
fn pads_bank0_gpio0_roundtrip() {
    let mut bus = Bus::new();
    // GPIO0 pad at 0x04.
    bus.write32(PADS_BANK0_BASE + 0x04, 0x56);
    assert_eq!(bus.read32(PADS_BANK0_BASE + 0x04), 0x56);
}

#[test]
fn pads_bank0_voltage_select_roundtrip() {
    let mut bus = Bus::new();
    bus.write32(PADS_BANK0_BASE + 0x00, 1);
    assert_eq!(bus.read32(PADS_BANK0_BASE + 0x00), 1);
}
...
fn resets_guard_held_uart0_reads_zero() {
    ...
    bus.write32(RESETS_BASE + 0x2000, 1 << RESET_UART0);
    assert_ne!(bus.resets_state & (1 << RESET_UART0), 0);
    // UARTFR should read 0 — guard intercepts before PL011 reset flags.
    assert_eq!(bus.read32(UART0_BASE + UARTFR), 0);
}

#[test]
fn resets_guard_held_spi0_discards_writes() {
    let mut bus = Bus::new();
    bus.write32(RESETS_BASE + 0x2000, 1 << RESET_SPI0);
    // Write SSPCR1 — must be dropped.
    bus.write32(SPI0_BASE + SSPCR1, SSPCR1_SSE | SSPCR1_LBM);
    // Release the peripheral.
    bus.write32(RESETS_BASE + 0x3000, 1 << RESET_SPI0);
    // CR1 should still be 0.
    assert_eq!(bus.read32(SPI0_BASE + SSPCR1), 0);
}
...
fn resets_guard_held_pwm_discards_tick_advance() {
    ...
    bus.write32(PWM_BASE + 0x00, CSR_EN);
    bus.write32(PWM_BASE + 0x10, 100);
    bus.write32(PWM_BASE + PWM_EN, 1);
    bus.write32(RESETS_BASE + 0x2000, 1 << RESET_PWM);
    bus.tick_peripherals(500);
    // Release and read CTR — must still be 0.
    bus.write32(RESETS_BASE + 0x3000, 1 << RESET_PWM);
    assert_eq!(bus.read32(PWM_BASE + 0x08), 0);
}
...
fn resets_guard_held_io_bank0_reads_zero() {
    ...
    bus.write32(IO_BANK0_BASE + 0x04, 0xABCD);
    bus.write32(RESETS_BASE + 0x2000, 1 << RESET_IO_BANK0);
    // Held: reads 0.
    assert_eq!(bus.read32(IO_BANK0_BASE + 0x04), 0);
    // Release and re-read — stored value still present.
    bus.write32(RESETS_BASE + 0x3000, 1 << RESET_IO_BANK0);
    assert_eq!(bus.read32(IO_BANK0_BASE + 0x04), 0xABCD);
}
...
fn adc_fifo_byte_write_does_not_pop_sample() {
    ...
    bus.write8(ADC_BASE + ADC_FIFO, 0xAA);
    // Full word read must still see the sample.
    let sample = bus.read32(ADC_BASE + ADC_FIFO);
    assert_ne!(
        sample & 0xFFFF, 0,
...
fn adc_fifo_halfword_write_does_not_pop_sample() {
    ...
    bus.write32(ADC_BASE + ADC_CS, CS_EN | CS_START_ONCE | (3 << 12));
    bus.tick_peripherals(500);
    bus.write16(ADC_BASE + ADC_FIFO, 0xBEEF);
    let sample = bus.read32(ADC_BASE + ADC_FIFO);
    assert_ne!(
        sample & 0xFFFF, 0,
