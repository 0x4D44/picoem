...
use rp2040_emu_tui::sim::{self, FirmwareBytes};
use rp2040_emu_tui::snapshot::Snapshot;
use rp2040_emu_tui::ui;
...
fn resolve_firmware_path(arg: &str) -> PathBuf {
    ...
    match arg {
        "blinky" => manifest_dir.join("../../roms/rp2040/blinky.bin"),
        path => PathBuf::from(path),
    }
}

fn bootrom_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../roms/rp2040/bootrom.bin")
}
...
mod tests {
    ...
    #[test]
    fn preset_alias_resolves_under_manifest_dir() {
        assert_eq!(
            resolve_firmware_path("blinky"),
            manifest_dir().join("../../roms/rp2040/blinky.bin")
        );
    }
