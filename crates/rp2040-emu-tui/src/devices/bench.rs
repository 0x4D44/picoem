...
//! `0x2003_FF00` at the start and end of each timed section:
//!
//! | Phase | Meaning                                          |
//! |-------|--------------------------------------------------|
...
//! # RP2040 note
//!
//! There is currently no RP2040 benchmark firmware — the panel will
//! sit at "waiting..." when running blinky. The poller infrastructure
//! is kept in place so an RP2040 benchmark ROM can be wired in later
//! without rewriting the sim-thread plumbing.

use std::time::Duration;

use rp2040_emu::Emulator;

use crate::firmware::REFERENCE_TABLE;
use crate::snapshot::{BenchmarkReport, BenchmarkSection};

/// Sentinel address — matches rp2350_emu_tui so the same benchmark-
/// firmware template can be adapted later.
