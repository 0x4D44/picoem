...
### Real RP2040 bootrom not vendored (blocks PicoGUS boot)

**Impact: HIGH.** Blocks the HLD's subjective-ear-test acceptance.

Discovered while smoke-testing `scripts/picogus_demo.sh` against
the real PicoGUS v4.0.0 firmware image. Our `roms/rp2040/bootrom.bin`
is a synthetic 8-byte stub generated by `roms/rp2040/gen_blinky.py`
— it maps an initial SP + reset vector that jumps straight to
flash. It does not:

- Execute the stock RP2040 boot2 (XIP_SSI configuration, flash
  access mode, stage2 copy-from-flash-to-SRAM).
- Provide the SDK bootrom function table (`rom_func_lookup`,
  `rom_data_lookup`, memset / memcpy / usb-boot helpers).

Symptom: `picogus_diff_rp2040 --flash picogus-v4.0.0.bin` stalls at
cycle 8209 (inside boot2's `PUSH {LR}` preamble), fires every
subsequent trace event at the stuck cycle, and the I2S capture
writes a 44-byte empty WAV. Stage 6 acceptance cannot be met
without resolving this.

Fix options (ranked):

1. **Vendor the real RP2040 bootrom** into `roms/rp2040/` and teach
   `picogus_diff_rp2040` to call `emu.load_bootrom(&bootrom)` when a
   `--bootrom <path>` flag is supplied (or default-search the file
   when present). The Raspberry Pi RP2040 bootrom source is at
   <https://github.com/raspberrypi/pico-bootrom-rp2040> under a
   permissive licence.
2. **Write a Rust-side boot2 shim** that does the flash-to-SRAM
   copy the SDK bootrom would do. Requires reading the linker
   script to find `__StackTop` and `__vector_table_in_ram`.
3. **Pre-process the firmware** to extract the post-boot2 payload
   and load it via `--image` straight into SRAM. Requires an
   LLDB-style parse of the firmware's vector table + BSS / data
   sections.

Option 1 is the cleanest. Estimated 1–2 hours to implement and test
once someone starts.
